...
ggplot() + 
  geom_sf(data = world_no_ant,
          fill = _________, color = _______, linewidth = _______) + #pick a color for the countries, the color of their borders, and the width of the borders
  theme_void()
```
...
ggplot() + 
  ...
  coord_sf(crs = "___________") +  #set to the robinson projection
  theme_void()
```
...
```{r}
only_ca <- lower_48 %>% 
  filter(NAME == "_________")  #what variable value do we want to filter for?
